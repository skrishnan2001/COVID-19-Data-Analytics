...
# # Reading the csv file from covid19.org for getting infomation about Total,Recovered and Active cases
...
def graph13():
    ...
    df = df.iloc[42:, :]
    x = df['Date']
    UP = df['UP'].iloc[-1] / 237.8827
    TG = df['TG'].iloc[-1] / 39.36273
    MP = df['MP'].iloc[-1] / 85.35897
    GJ = df['GJ'].iloc[-1] / 63.8724
    MH = df['MH'].iloc[-1] / 123.1442
    KL = df['KL'].iloc[-1] / 35.69944
    KA = df['KA'].iloc[-1] / 67.56269
    RJ = df['RJ'].iloc[-1] / 81.03269
    AP = df['AP'].iloc[-1] / 53.90339
    TN = df['TN'].iloc[-1] / 77.84127
    DL = df['DL'].iloc[-1] / 18.71092
    States = ['UP', 'TG', 'MP', 'GJ', 'MH', 'KL', 'KA', 'RJ', 'AP', 'TN', 'DL']
    cases_per_million = []
